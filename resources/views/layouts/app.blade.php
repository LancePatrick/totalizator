...
@php
    ...
    $kycApproved = $role !== 'player' || $user?->kyc_status === 'approved';

    $safeRoute = function ($routeName, $fallback = 'dashboard') {
        return Route::has($routeName) ? route($routeName) : route($fallback);
    };

    $playerProtectedHref = function ($routeName) use ($safeRoute, $kycApproved) {
        if (!$kycApproved) {
            return $safeRoute('player.kyc.index');
        }
    ...
    $menu = match ($role) {
        ...
        default => [
            ['label' => 'Dashboard', 'href' => $playerProtectedHref('player.dashboard'), 'icon' => 'home', 'active' => request()->routeIs('player.dashboard')],
            ['label' => 'Wallet', 'href' => $playerProtectedHref('player.wallet.index'), 'icon' => 'wallet', 'active' => request()->routeIs('player.wallet.*')],
            ['label' => 'Play Game', 'href' => $playerProtectedHref('player.game.index'), 'icon' => 'game', 'active' => request()->routeIs('player.game.*')],
            ['label' => 'KYC Verification', 'href' => $safeRoute('player.kyc.index'), 'icon' => 'shield', 'active' => request()->routeIs('player.kyc.*')],
            ['label' => 'Transactions', 'href' => $playerProtectedHref('player.wallet.index'), 'icon' => 'document', 'active' => request()->routeIs('player.transactions.*')],
        ],
    };
    ...
    $promoHref = $role === 'player'
        ? ($kycApproved ? $safeRoute('player.game.index') : $safeRoute('player.kyc.index'))
        : route('dashboard');

    $promoButtonText = $role === 'player'
        ? ($kycApproved ? 'Play Now' : 'Verify KYC')
        : 'Go to Dashboard';
@endphp
    ...
    <div class="app-shell">
        <aside class="sidebar">
            <div class="sidebar-inner">
                ...
                @if($role === 'player' && !$kycApproved)
                    <div class="mx-4 mb-4 rounded-2xl border border-yellow-300 bg-yellow-50 px-4 py-3 text-xs font-black text-yellow-800">
                        KYC required before using wallet, cash in, cash out, and games.
                    </div>
                @endif
                ...
                <div class="promo-card">
                    ...
                    <p>
                        ...
                        @else
                            ...
                            @else
                                Verify your KYC first to unlock games and wallet features.
                            @endif
                        @endif
                    </p>

                    <a href="{{ $promoHref }}" class="promo-btn">
                        {{ $promoButtonText }}
                    </a>
                </div>
